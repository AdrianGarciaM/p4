...
public class ChessBoardRenderer {
	...
	private Chess.ChessPiece movingPiece;
        Chess.ChessPieceImplementation.Color turn = Chess.ChessPieceImplementation.Color.WHITE;
	...
	public void setMovingPiece(ChessPiece movingPiece) 
        {
            if(movingPiece != null)
            {
                if(movingPiece.getColor().equals(turn))
	...
	boolean movePieceTo(Canvas canvas, double x, double y) {
		...
		if (boardBounds.contains(x, y)) {
			...
			if (movingPiece.canMoveToPosition(position, board))
                        {
                            // Una vez que la pieza se va a mover, se puede cambiar el turno.
                            if(turn == Chess.ChessPieceImplementation.Color.WHITE)
                                turn = Chess.ChessPieceImplementation.Color.BLACK;
                            else
                                turn = Chess.ChessPieceImplementation.Color.WHITE;
	...
	boolean containsKing(ChessPiece.Color aColor) {
        ...
        void loadGame(File location)
        {
            board.loadFromFile(location);
        }
        
        void saveGame(File location)
        {
